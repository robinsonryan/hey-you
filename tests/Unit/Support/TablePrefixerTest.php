...
it('prefixes table names with configured prefix', function (): void {
...
it('returns table name unchanged when prefix is null', function (): void {
    config(['heyyou.table_prefix' => null]);

    expect(TablePrefixer::prefix('parties'))->toBe('parties');
});

it('returns table name unchanged when prefix is empty string', function (): void {
    config(['heyyou.table_prefix' => '']);

    expect(TablePrefixer::prefix('parties'))->toBe('parties');
});

it('allows custom prefixes', function (): void {
